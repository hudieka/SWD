...
uint8_t swd_read_core_register(uint32_t n, uint32_t *val) {
    int i = 0, timeout = 100;
    if (!swd_write_word(DCRSR_ADDR, n)) {
        return 0;
    }
    ...
    for (i = 0; i < timeout; i++) {
        ...
        if (!swd_read_word(DHCSR_ADDR, val)) {
            return 0;
        }
    ...
    if (!swd_read_word(DCRDR_ADDR, val)) {
        return 0;
    }
...
uint8_t swd_write_core_register(uint32_t n, uint32_t val) {
    int i = 0, timeout = 100;
    if (!swd_write_word(DCRDR_ADDR, val))
        return 0;

    if (!swd_write_word(DCRSR_ADDR, n | REGWnR)) {
        return 0;
    }
...
/*  The argument delay_time specifies the length of the pause 
	and is measured in number of system_ticks */
#pragma optimize=none
void os_dly_wait(uint32_t delay_time)
{
