...
#include "MKL25Z4.H"                             // Debug Unit Cortex-M Processor Header File
...
// nRESET Pin                   PTD3
#define PIN_nRESET_PORT         PORTD
#define PIN_nRESET_GPIO         PTD
...
static  uint32_t PIN_SWDIO_IN      (void) {
	//yangliang
	return ((PIN_SWDIO_IN_GPIO->PDIR >> PIN_SWDIO_IN_BIT)&0x01);
...
static  void     PIN_SWDIO_OUT     (uint32_t bit) {
  //yangliang
	if(bit != 0)
	{
			//PIN_SWDIO_OUT_GPIO->PDOR |= (1<<PIN_SWDIO_OUT_BIT);
		PIN_SWDIO_OUT_GPIO->PSOR |= (1<<PIN_SWDIO_OUT_BIT);
	}
	else
	{
...
static void     PIN_nRESET_OUT (uint32_t bit) { 
  //yangliang
	if(bit != 0)
	{
			PIN_nRESET_GPIO->PDOR |= (1<<PIN_nRESET_BIT);
	}
	else
	{
			PIN_nRESET_GPIO->PDOR &= ~(1<<PIN_nRESET_BIT);
...
static inline void LED_CONNECTED_OUT (uint32_t bit) {
  //yangliang
	if(bit != 0)
	{
			LED_CONNECTED_GPIO->PDOR |= (1<<LED_CONNECTED_BIT);
	}
	else
	{
			LED_CONNECTED_GPIO->PDOR &= ~(1<<LED_CONNECTED_BIT);
